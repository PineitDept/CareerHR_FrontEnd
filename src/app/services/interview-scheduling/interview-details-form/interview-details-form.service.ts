...
export class InterviewDetailsFormService {
  ...
  previewFormRound<T>(round: number, userId: string): Observable<T> {
    return this.api.get<T>(`${this.baseForm}/round/${round}/userId/${userId}/preview`, { withAuth: true, loading: true });
  }
}
