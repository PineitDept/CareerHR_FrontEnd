...
import {
  ...
  IInterviewrWithPositionsDto,
  InterviewrScreeningRow,
...
@Component({
  ...
  styleUrl: './interviewer-list.component.scss'
})
export class InterviewerListComponent extends BaseGeneralBenefitsComponent<IInterviewrWithPositionsDto> {
  @ViewChild('tables', { static: false }) tables!: TablesComponent;
  @Output() toggleRequested = new EventEmitter<{ row: any; next: boolean }>();

  hasMoreData = true;
  currentPage = 1;
  ScreenRows: InterviewrScreeningRow[] = [];
  isAddingRow = false;
  ...
  override ngOnInit(): void {
    this.generalBenefitsService.setBenefitType('interviewer-list');
    this.loadUsers();
    // super.ngOnInit();
  }

  handleEditRow(row: InterviewrScreeningRow): void {
    const id = row.idEmployee;
    const payload: Partial<IInterviewrWithPositionsDto> = {
    ...
    //       this.duplicateRowIndex = rows.findIndex(row => row.fullName.trim().toLocaleLowerCase() === name.trim().toLocaleLowerCase());
    //     }, 100)
  ...
  onInlineSave(payload: any) {
    ...
    this.generalbenefitsService.createBenefit(payload).subscribe({
      ...
      error: (err) => {
        ...
        const rows = [...this.ScreenRows];
        this.duplicateRowIndex = rows.findIndex(row => row.fullName.trim().toLocaleLowerCase() === payload.fullName.trim().toLocaleLowerCase());
      }
    });
  }
  ...
  loadUsers() {
    this.generalBenefitsService.getBenefitsWeb<IInterviewrWithPositionsDto[]>(this.currentFilterParams).subscribe({
      next: (res) => {
  ...
  onUserToggleRequested({
    ...
    if (this.isAddingRow && !row.idEmployee) {
      checkbox.checked = checked;
      if ('isActive' in row) row.isActive = checked;
      if ('activeStatus' in row) row.activeStatus = checked;
    } else {
      this.generalBenefitsService.toggleStatus(row.idEmployee).subscribe({
        next: () => {
          checkbox.checked = checked;
          if ('isActive' in row) row.isActive = checked;
          if ('activeStatus' in row) row.activeStatus = checked;
